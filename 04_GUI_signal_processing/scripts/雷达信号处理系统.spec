...
"""
macOS 版 PyInstaller 打包配置 — 雷达信号处理系统 (模块04)

用法:
    cd GUI_signal_processing
    source venv/bin/activate
    pyinstaller --clean scripts/雷达信号处理系统.spec

输出: dist/雷达信号处理系统.app
"""
...
# 自动查找 .so 文件名
cpp_module = glob.glob(GUI_ROOT + '/lib/signal_processing_cpp*.so')
if not cpp_module:
    print("[错误] 未找到 signal_processing_cpp*.so，请先编译 C++ 绑定")
