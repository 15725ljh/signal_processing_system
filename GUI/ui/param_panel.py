...
class DerivedLabel(QLabel):
    ...
    def set_text(self, text):
        """Set the label text. Named set_text to avoid shadowing QWidget.update()."""
        self.setText(text)
...
class ParamPanel(QWidget):
    ...
    _WAVEFORM_PARAMS = [
        ("── Case1: 固定跳频 ──", [
            ...
            ("频率步进 delta_f (Hz)", "waveform.case1_freq_hop.delta_f", 40e6, False, "", 1e6),
        ]),
        ("── Case2: 随机相位 ──", [], "固定载频，每个脉冲附加\n[0, 2π) 均匀随机相位，无需额外参数"),
        ("── Case3: PRI抖动 ──", [
            ("标称脉冲间隔 prt (s)", "waveform.case3_pri_jitter.prt", 1000e-6, False, "", 1e-6),
            ("信号幅度", "waveform.case3_pri_jitter.amp", 1.0, False, "", 0.1),
            ("抖动范围 (±μs)", "waveform.case3_pri_jitter.jitter_us", 20, True, "", 1),
        ]),
        ("── Case4: 混合波形 ──", [
            ("频率步进 delta_f (Hz)", "waveform.case4_hybrid.delta_f", 40e6, False, "", 1e6),
            ("载频分组数 fcnum", "waveform.case4_hybrid.fcnum", 16, True, "", 1),
            ("标称脉冲间隔 prt (s)", "waveform.case4_hybrid.prt", 1000e-6, False, "", 1e-6),
            ("信号幅度", "waveform.case4_hybrid.amp", 1.0, False, "", 0.1),
            ("抖动范围 (±μs)", "waveform.case4_hybrid.jitter_us", 20, True, "", 1),
        ]),
        ("── Case5: 复合波形 ──", [
            ("跳频点数 N", "waveform.case5_combined.N", 10, True, "", 1),
            ("频率步进 delta_f (Hz)", "waveform.case5_combined.delta_f", 40e6, False, "", 1e6),
        ]),
    ]
    ...
    def _create_lock_button(self):
        ...
        return self._wrap_row_as_widget(row)

    @staticmethod
    def _wrap_row_as_widget(layout):
        w = QWidget()
        w.setLayout(layout)
        return w
    ...
    def _create_waveform_params_group(self):
        group = QGroupBox("波形专属参数")
        layout = QVBoxLayout(group)
        for sep_text, params, *rest in self._WAVEFORM_PARAMS:
            sep = QLabel(sep_text)
            ...
            if desc:
                ...
                desc_lbl.setStyleSheet("color: #7f8c8d; font-size: 11px; padding: 2px 12px;")
