...
"""
Windows 版 PyInstaller 打包配置 — 雷达干扰生成系统

用法:
    cd GUI_jamming
    venv\Scripts\activate
    pyinstaller --clean scripts/雷达干扰生成系统_win.spec

输出: dist/雷达干扰生成系统/雷达干扰生成系统.exe
"""
...
cpp_module = glob.glob(GUI_ROOT + '/lib/jamming_cpp*.pyd')
if not cpp_module:
    print("[错误] 未找到 jamming_cpp*.pyd，请先编译 C++ 绑定")
    print("       在 Developer Command Prompt 中运行: scripts\\build.bat")
    sys.exit(1)

print(f"[信息] C++ 模块: {cpp_module[0]}")

# MinGW UCRT64 运行时 DLL (jamming_cpp.pyd 的依赖)
