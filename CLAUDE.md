...
- **PyInstaller excludes**: Only exclude clearly unnecessary packages (`tkinter`, `IPython`, `jupyter`, `notebook`, `matplotlib`, `pandas`, `flask`, `django`, `pytest`, `lib2to3`, `setuptools`, `pip`). Note: `matplotlib` is in `requirements.txt` for development use but excluded from exe to reduce size. Do NOT exclude `unittest`, `pydoc`, `multiprocessing`, `inspect`, `xml` etc. — they are needed by numpy/scipy/pyqtgraph dependency chains.
