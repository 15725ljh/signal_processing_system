...
cmd_cpp() {
    ...
    NLOHMANN_INC="$THIRD_PARTY/nlohmann"

    if [ ! -f "$MODULE03_LIB" ]; then
        echo "[错误] 静态库未构建,请先运行: cd 03_jamming_detection_suppression && mkdir -p build && cd build && cmake .. && cmake --build . --target detection_core"
        exit 1
    fi
...
cmd_app() {
    echo "[3/3] 打包 .app..."
    rm -rf build/ dist/
    $PY -m PyInstaller --clean "$SCRIPT_DIR/雷达干扰识别系统.spec" 2>&1 | tail -3
    rm -rf "dist/雷达干扰识别系统.app/Contents/Resources/core/__pycache__"
    echo "[完成] dist/雷达干扰识别系统.app"
}
