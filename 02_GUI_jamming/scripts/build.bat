...
echo === [1/3] 编译 C++ 绑定 (jamming_cpp.pyd) ===
...
set MODULE02_LIB=%GUI_ROOT%\..\02_jamming_generation\build\libjamming_core.a
if not exist "%MODULE02_LIB%" (
    echo [错误] 未找到 libjamming_core.a，请先构建模块02
    echo        cd 02_jamming_generation ^&^& mkdir build ^&^& cd build ^&^& cmake .. -G "MinGW Makefiles" ^&^& cmake --build . --target jamming_core
    exit /b 1
)
...
g++ -O3 -std=c++17 -shared ^
    ...
    -I"%GUI_ROOT%\..\02_jamming_generation\include" ^
    "%GUI_ROOT%\..\02_jamming_generation\bindings\jamming_bind.cpp" ^
    "%MODULE02_LIB%" ^
...
echo.
echo ========================================
echo   完成: dist\雷达干扰生成系统\雷达干扰生成系统.exe
echo ========================================
pause
