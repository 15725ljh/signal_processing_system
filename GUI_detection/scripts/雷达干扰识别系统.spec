...
"""
macOS 版 PyInstaller 打包配置 — 雷达干扰识别与抑制系统

用法:
    cd GUI_detection
    source venv/bin/activate
    pyinstaller --clean scripts/雷达干扰识别系统.spec

输出: dist/雷达干扰识别系统.app
"""
...
# 自动查找 .so 文件名
cpp_module = glob.glob(GUI_ROOT + '/detection_cpp*.so')
if not cpp_module:
    print("[错误] 未找到 detection_cpp*.so，请先编译 C++ 绑定")
